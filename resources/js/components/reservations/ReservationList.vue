        ...
        <div class="mt-6 space-y-4">
            <div v-if="reservationItems.length" class="grid gap-3">
                <div
                    ...
                    :key="reservation.id"
                    class="brand-card-soft flex items-start justify-between gap-4"
                >
                    <div class="space-y-1">
                        <p
                            class="text-sm font-semibold text-[color:var(--brand-ink-strong)]"
                        >
                            {{ formatReservationDate(reservation.date) }}
                        </p>
                        <p
                            class="text-sm text-[color:var(--brand-ink-muted)]"
                        >
                    ...
                    <button
                        type="button"
                        class="brand-link border-red-200 text-xs text-red-600 hover:border-red-300"
                ...
                <div
                    v-if="totalPages > 1"
                    class="flex items-center justify-between text-sm text-[color:var(--brand-ink-muted)]"
                >
                    <Link
                        :href="prevLink ?? ''"
                        preserve-scroll
                        preserve-state
                        class="brand-link text-xs"
                        :class="
            ...
            <p v-else class="text-sm text-[color:var(--brand-ink-muted)]">
